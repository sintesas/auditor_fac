					...
					<table id="datatable1" class="table table-striped table-hover">
						...
						<tbody>
							...
							<tr>
								...
								<th class="factivilidadtecnica">
									{{($demanda->Severidad==1)?'Catastrófico':($demanda->Severidad==2)?'Crítico':($demanda->Severidad==3)?'Marginal':$demanda->Severidad}}
								</th>
								<th class="factivilidadtecnica">
									{{($demanda->OcurrenciaFalla==1)?'Frecuente':($demanda->OcurrenciaFalla==2)?'Probable':($demanda->OcurrenciaFalla==3)?'Ocasional':($demanda->OcurrenciaFalla==4)?'Remoto':($demanda->OcurrenciaFalla==5)?'Improbable':$demanda->OcurrenciaFalla}}
								</th>
								<th class="factivilidadtecnica">
									{{($demanda->Complejidad==1)?'Alta':($demanda->Complejidad==2)?'Media':($demanda->Complejidad==3)?'Baja':$demanda->Complejidad}}
								</th>
								<th class="uma">
									{{($demanda->PrioridadUMA==1)?'Alta (Ocasiona AOG)':($demanda->PrioridadUMA==2)?'Media (Ocasiona PPA)':($demanda->PrioridadUMA==3)?'Baja (No afecta la operación)':$demanda->PrioridadUMA}}
								</th>
								<th>{{$demanda->TOTAL}}</th>
								...
								<th class="valoracioneconomica">{{(isset($demanda->AñoVH))?$demanda->AñoVH:''}}</th>
								<th class="valoracioneconomica">@if($demanda->ValorUnitario) ${{number_format($demanda->ValorUnitario, 0, '', '.')}}@endif</th>
								<th class="valoracioneconomica">{{(isset($demanda->AñoVUC))?$demanda->AñoVUC:''}}</th>
								...
								<th class="valoracioneconomica">{{(isset($demanda->AñoProximaEntrega))?$demanda->AñoProximaEntrega:''}}</th>
