...
use App\CaracteristicaEmpresa;
use App\ClienteFfmm;
use App\ProductoFfmm;

//Caracteristicas Empresa
use App\TamanoEmpresa; 
use App\CriterioFinanciero;

// mercado
use App\Mercado;

use App\SectorMercado;
use App\CategoriaTipo;
use App\SubcategoriaTipo;


// materias primas
use App\ListadoMateriasPrimas;
use App\MateriaPrima;


// producto ofrecido
use App\ProductoOfrecido;
use App\ServicioOfrecido;
use App\Empresa;
use App\DemandaPotencial;


// Tecnologias
use App\TecnologiaEmpresa;
use App\AntecedenteAutor;
use App\TecnologiaSuCliente;
use App\ProductoSector;
use App\Desarrollado;
use App\AgenteDesarrollador;

// Produccion Empresa
use App\ProduccionEmpresa;
use App\ProduccionTercerizacion;
use App\MaquinariaEquipoProduccion;
...
class InformacionProduccionController extends Controller
{
    ...
    public function edit($IdEmpresa){
        ...
        return view('fomento.empresas.ver_detalle_informacion_produccion')->with('empresa', $empresa)->with('clientesffmm', $clientesffmm)->with('sectoresMercado', $sectoresmercado)->with('categoriasTipo', $categoriastipo)->with('subcategoriasTipo', $subcategoriastipo)->with('listadomateriasprimas', $listadomateriasprimas)->with('tamanoempresa', $tamanoempresa)->with('criteriofinanciero', $criteriofinanciero)->with('demandapotencial', $demandapotencial)->with('productosffmm', $productosffmm)->with('infomercado', $infomercado)->with('materiasprimas', $materiasprimas)->with('maquinarias', $maquinarias)->with('producciontercerizacion', $producciontercerizacion)->with('produccionempresa', $produccionempresa)->with('caracteristica', $caracteristica)->with('productosofrecidos', $productosofrecidos)->with('serviciosofrecidos', $serviciosofrecidos)->with('productossector', $productossector)->with('antecedentesautor', $antecedentesautor)->with('desarrollados', $desarrollados)->with('tecnologiassucliente', $tecnologiassucliente)->with('agentedesarrollador', $agentedesarrollador)->with('tecnologia', $tecnologia);
