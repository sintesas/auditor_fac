...
class SeguimientosExport implements FromCollection,  WithHeadings
{
  ...
  /**DB::table('users')
    ...
    */
    ...
    {
    return DB::table('exportseguimiento')->get();
        /*return DB::table('AU_Reg_CausasRaizTareas as tareaCausa')
            ->select(DB::raw(
				...
				IsNULL((select CAST((sum(rs.Porcentaje)/COUNT(rcr.IdAnotacion)) AS INT) from AU_Reg_Anotaciones as ra inner join AU_Reg_CausasRaiz as rcr on rcr.IdAnotacion = ra.IdAnotacion inner join AU_Reg_CausasRaizTareas as rcrt on rcrt.IdCausaRaiz = rcr.IdCausaRaiz left join
        (Select sg.* from (SELECT IdTareaCausa, MAX(FechaSeguimiento) as maxfecha from AU_Reg_Seguimiento group by IdTareaCausa) as fl
           inner join AU_Reg_Seguimiento as sg
           on sg.IdTareaCausa = fl.IdTareaCausa AND sg.FechaSeguimiento = fl.maxfecha)
         as rs on rs.IdTareaCausa = rcrt.IdTarea where rcr.IdAnotacion = causa_seguimiento.IdAnotacion ),0)
